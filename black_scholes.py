...
class OptionPricingComparison:
    ...
    def plot_price_sensitivity(self, param_name: str, param_range: np.ndarray, option_type: str = 'call'):
        """
        ...
        for param_value in param_range:
            if param_name == 'S':
                bs = BlackScholes(param_value, self.K, self.T, self.r, self.sigma)
            elif param_name == 'K':
                bs = BlackScholes(self.S, param_value, self.T, self.r, self.sigma)
            elif param_name == 'T':
                bs = BlackScholes(self.S, self.K, param_value, self.r, self.sigma)
            elif param_name == 'r':
                bs = BlackScholes(self.S, self.K, self.T, param_value, self.sigma)
            elif param_name == 'sigma':
                bs = BlackScholes(self.S, self.K, self.T, self.r, param_value)
            else:
        ...
        plt.ylabel(f'{option_type.title()} Option Price')
        plt.title(f'{option_type.title()} Option Price vs {param_name}')
        plt.grid(True, alpha=0.3)
        plt.show()
    
    def plot_greeks(self, param_name: str, param_range: np.ndarray, greek_name: str, option_type: str = 'call'):
        """
        ...
        Args:
            param_name (str): Name of parameter to vary
            param_range (np.ndarray): Range of parameter values
            greek_name (str): Name of Greek to plot
            option_type (str): Type of option ('call' or 'put')
        """
        greeks_values = []
        ...
        for param_value in param_range:
            ...
            greeks = bs.greeks()
            greek_key = f'{option_type}_{greek_name}'
            greeks_values.append(greeks[greek_key])
        
        plt.figure(figsize=(10, 6))
        plt.plot(param_range, greeks_values, 'r-', linewidth=2)
        plt.xlabel(f'{param_name}')
        plt.ylabel(f'{greek_name.title()} ({option_type})')
        plt.title(f'{greek_name.title()} vs {param_name}')
        plt.grid(True, alpha=0.3)
        plt.show()
